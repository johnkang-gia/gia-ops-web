...
export function extractTargetRange(text: string, baseDate: Date): TargetRange | null {
  ...
  // 사이를 잇는 말은 물결·붙임표만이 아닙니다. "9월 21일**부터** 23일까지" 처럼 글자로
  // 적는 경우가 더 흔한데, 이것을 못 읽어서 시작일이 **글이 온 날**로 잡혔습니다.
...
export type AttendanceEntry = {
  ...
  // ── 출처를 «어느 대화방의 누구»까지 ────────────────────────────────────────
  //
  // 이름이 겹치거나 애매할 때, 사람이 맞는지 가리는 근거는 결국 «누가 어디서 보낸 글인가»
  // 입니다. 「이연우」가 둘이어도, 이연우 어머니 채널에서 온 글이면 그 아이입니다.
  // 원문만 보여주면 이 판단을 할 수가 없어, 담당자가 토들을 따로 열어 찾아야 했습니다.
  /** 토들 채널 이름(대개 학부모 채널). 구글챗은 비어 있습니다. */
  channelLabel?: string | null;
  /** 구글챗에서 이 글을 쓴 사람. 토들은 비어 있습니다. */
  senderName?: string | null;
  /** 원문으로 바로 가는 주소. 있으면 창에서 [원문 열기] 버튼이 생깁니다. */
