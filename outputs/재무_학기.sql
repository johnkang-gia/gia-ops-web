-- ═══════════════════════════════════════════════════════════════════════════
-- 재무 학기 — Supabase SQL Editor 붙여넣기용
--
-- 지금 등록된 학비외 항목과 발행된 청구서를 전부 **2026 정규학기**로 넘깁니다.
-- 여러 번 실행해도 됩니다.
-- ═══════════════════════════════════════════════════════════════════════════
...
values ('20260902260000', 'fee_terms')
on conflict (version) do nothing;

-- 확인 — 학기별 항목·청구서 수. '학기 없음' 이 0이어야 합니다.
select coalesce(t.name, '(학기 없음)') as 학기,
       count(distinct i.id) as 항목수,
       count(distinct v.id) as 청구서수
from public.fee_terms t
full join public.fee_items i on i.fee_term_id = t.id
full join public.invoices  v on v.fee_term_id = t.id
