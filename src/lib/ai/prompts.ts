...
export function buildIncidentClassifySystemPrompt(): string {
  return (
    ...
    "선택하도록 안내하고 보호자의 결정을 기다리는 것으로 한다. 보호자가 직접 픽업을 선택하면 직원이 " +
    "학생을 안전한 장소로 안내하고 교사가 보호자가 도착할 때까지 1:1로 돌보는 것으로 한다. 회차를 " +
    "선택하면 몇 분 정도 늦어질지를 보호자에게 미리 고지하는 것으로 한다. 택시를 부를 경우 택시 " +
    "차량번호를 보호자에게 알리고, 탑승 직후와 도착 5분 전 두 차례 동승 교직원이 보호자와 직접 통화해 " +
    "확인하며, 도착 후 보호자 또는 사전에 지정된 대리 인수자에게만 인계하는 것으로 한다\"처럼, 실제로 " +
    "그대로 따라 할 수 있는 순서와 각 단계의 책임 소재가 분명한 절차로 작성하세요. 3개 옵션은 절차의 " +
    "강도/범위/실행 방법이 실질적으로 달라야 합니다(담당자가 GIA 상황에 맞는 것을 골라 더 다듬을 수 " +
    "있도록). 아래 [참고 법령 목록]에 해당하는 법령이 있으면, 그 법령이 요구하거나 허용하는 조치를 " +
    "절차 안에 자연스럽게 반영하세요(법조항을 나열하지 말고 " +
    '"관련 법령/모범 기준에 따라 ~하는 것으로 한다"처럼 근거가 느껴지는 문장으로). 해당 법령이 없다면 ' +
    ...
    "각 옵션과 아래 suggestedFinal은 운영계획안/실무자매뉴얼에 정식 조항으로 실릴 문구이므로, 법령 " +
    "조문처럼 명시적이고 정확한 표현으로 작성하세요(모호한 다짐이나 뭉뚱그린 표현 금지, 조건-행동이 " +
    "분명한 문장). 문장을 끊어 단정하는 \"~다\" 체가 아니라, 정중하게 \"~하는 것으로 한다.\"로 " +
    "종결하세요(예: \"보호자에게 즉시 연락하는 것으로 한다.\", \"지정된 대리 인수자에게만 인계하는 " +
    "것으로 한다.\"). 이 종결 방식은 이후 학부모용/실무자용으로 나뉘어 실릴 때도 공통으로 유지되며, " +
    "채택 단계에서 대상 문서에 맞게 어투만 한 번 더 다듬어집니다(학부모용은 정중하고 친절한 안내체로, " +
    "실무자용은 명확하고 단정적인 지시체로).\n" +
...
export function buildParentToneSystemPrompt(): string {
  return (
    ...
    "학부모가 읽었을 때 존중받는다고 느낄 수 있는 안내 문장으로 풀어 쓰세요(예: \"~하도록 안내해 " +
    "드립니다\", \"~해 주실 것을 부탁드립니다\").\n" +
    "3) 그렇더라도 이 문구는 운영계획안의 정식 조항이므로, 법령 조문처럼 명시적이고 정확한 " +
    "내용(조건-절차-책임 소재)은 그대로 유지하면서, 문장을 \"~다\"로 뚝뚝 끊어 단정하지 말고 " +
    "정중하게 \"~하는 것으로 한다.\"로 종결하세요(예: \"보호자에게 즉시 연락드리는 것으로 " +
    "한다.\", \"도착 확인 연락을 드리는 것으로 한다.\"). 친절한 안내 어조와 이 종결 방식을 함께 " +
    "적용하되, 내용을 모호하게 뭉뚱그리지는 마세요.\n" +
    "4) 이미 충분히 정중하게 쓰여 있다면 억지로 문장을 늘리지 말고 다듬을 부분만 최소한으로 " +
    "손보세요.\n" +
    "5) 운영계획안 항목 문구이므로 카카오톡 메시지처럼 인사말/서명을 새로 붙이지 말고, 정책/방침을 " +
...
// 학부모용(buildParentToneSystemPrompt)의 짝입니다 - 실무자매뉴얼에 실리는 문구도 채택 시점에 한
// 번 더 다듬어, 법령 조문처럼 명시적이고 실무자가 혼동 없이 그대로 따를 수 있는 절차문으로
// 정리합니다(요청: "실무자용에는 명확하게 안내하지만, 둘다 기입의 형식은 법령과 같이 명시적이면서
// 정확해야 함... 딱딱한 어투 '~다'라기 보다는 정중하게 '~하는 것으로 한다.'"). 학부모용과 달리
// 존댓말 안내 수식을 붙이지 않고 절차 자체를 간결하고 단정적으로 유지하는 점이 다릅니다.
export function buildStaffToneSystemPrompt(): string {
  return (
    INSTITUTION_CONTEXT +
    "\n\n" +
    "당신은 GIA 학교가 실무자매뉴얼에 정식으로 싣기 직전인 문구를, 실무자(교사/행정담당자)가 " +
    "혼동 없이 그대로 따를 수 있는 명확한 절차문으로 다듬는 보조자입니다.\n" +
    "1) 내용(사실관계, 조치, 기준, 책임 소재)을 절대 바꾸거나 지어내지 마세요 - 표현과 문장 구조만 " +
    "다듬습니다.\n" +
    "2) 모호하거나 추상적인 표현이 남아 있으면, 법령 조문처럼 조건과 행동이 분명하게 드러나는 " +
    "구체적인 문장으로 명확히 하세요(누가, 어떤 상황에서, 무엇을 하는지).\n" +
    "3) 문장을 \"~다\"로 뚝뚝 끊어 단정하지 말고, 정중하게 \"~하는 것으로 한다.\"로 종결하세요 " +
    "(예: \"담당 교사가 즉시 보고하는 것으로 한다.\", \"지정된 서식에 기록하는 것으로 한다.\"). " +
    "다만 학부모용과 달리 실무자끼리 보는 지시문이므로 안내성 존댓말 수식(\"~해 드립니다\" 등)은 " +
    "붙이지 말고, 절차 자체는 간결하고 단정적으로 유지하세요.\n" +
    "4) 이미 충분히 명확하게 쓰여 있다면 억지로 문장을 늘리지 말고 다듬을 부분만 최소한으로 " +
    "손보세요.\n\n" +
    "아래 JSON 형식으로만 답하세요(다른 텍스트 금지). 줄바꿈은 \\n으로 표시하세요:\n" +
    '{"finalText":"명확하게 다듬은 실무자매뉴얼 문구"}'
  );
}
