...
export type ShuttleBoarding = {
  ...
  // 배정된 노선 그대로입니다.
  /** shuttle-route-ok: 표의 칸 모양만 적는 자리입니다 - 판정은 @/lib/shuttleRoute 가 합니다. */
...
export type FeeItem = {
  ...
  id: string;
  /**
   * **사람이 읽는 고유 번호**(분류-일련번호, 예: 「교재-004」).
   *
   * 이름이 같은 항목이 넷 있습니다(학년별 중국어 교재 — 다른 것은 한국어 이름뿐). 화면에
   * 이름만 뜨면 넷 중 어느 것을 고르는지 사람이 알 수 없고, 실제로 하나를 「이미 받음」으로
   * 적었더니 넷이 모두 잠겼습니다. 번호는 겹치지 않습니다.
   *
   * 데이터베이스가 넣을 때 저절로 붙입니다. 옛 줄에는 아직 없을 수 있습니다.
   */
  code?: string | null;
  category: string;
   ...
   * 대상 — **누구를 위한 항목인가.**
   *
   * 자동으로 붙는지와는 다릅니다(auto_apply). 3학년 중국어는 3학년 것이지만 3학년 전원이
   * 사는 것은 아닙니다. 비어 있으면 '개별 지정' 입니다.
   */
   ...
   * 대상의 범위.
   *
   *   개별     아무에게도 자동으로 안 붙음 (인보이스 표에서 한 명씩)
   *   부서전체 이 항목의 부서 전원 (교복). 학년이 늘어도 저절로 따라옵니다
   *   학년     default_grades 의 학년 전원
   *   반       그 학년의 그 반만
   *
   ...
   * 대상 전원에게 자동으로 붙일지.
   *
   * true  = 3학년 공통 교과서 — 3학년이면 전원
   * false = 3학년 중국어      — 3학년 중에 고른 아이만 (대상은 표에서 열을 세우는 데 쓰임)
   *
   * 칸이 없던 시절 자료는 undefined 인데, 그때는 **true 로 봅니다**(지금 붙어 있는 것이
   * 그대로 유지되어야 합니다).
   */
