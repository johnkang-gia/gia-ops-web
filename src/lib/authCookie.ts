...
/** 승인 확인을 통과했을 때 발급하는 캐시 쿠키 값(userId + 발급시각 + 서명). */
export function makeAuthOkCookieValue(userId: string): string {
...
export function isAuthOkCookieValid(userId: string, cookieValue: string | undefined): boolean {
  ...
  const [cookieUserId, issuedAtStr, sig] = parts;
  if (cookieUserId !== userId) return false;
  const issuedAt = Number(issuedAtStr);
  if (!Number.isFinite(issuedAt)) return false;
  if (Math.floor(Date.now() / 1000) - issuedAt > TTL_SECONDS) return false;

  const expected = sign(`${cookieUserId}:${issuedAtStr}`);
