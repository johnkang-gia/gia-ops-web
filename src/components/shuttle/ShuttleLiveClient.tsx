...
function LiveRouteCard({
  ...
  if (completed) {
    ...
    /**
     * **한 번도 안 온 것과 오다가 늦는 것은 다릅니다.**
     *
     * 예전에는 둘 다 「수신 지연」이었습니다. 그래서 GPS를 켠 적이 없는 차(기기는 48대 전부
     * 발급돼 있습니다)가 사람이 출발을 누르는 순간 「운행중 · 수신 지연」으로 떴고, 보는
     * 사람은 **연결은 됐는데 신호가 늦는 것**으로 읽었습니다. 실제로는 오늘 한 건도 들어온
     * 적이 없습니다.
     *
     * 이 차이가 중요한 이유: 「지연」이면 기다리면 되지만, 「신호 없음」이면 기사님께 켜달라고
     * 연락해야 합니다. 할 일이 정반대인데 같은 글자를 보고 있었습니다.
     */
    statusLabel = ping == null ? "운행중 · GPS 신호 없음(사람이 체크)" : fresh ? "운행중" : "운행중 · 수신 지연";
    statusColor = "#1d4ed8";
