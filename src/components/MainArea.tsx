...
// 쓰고 있어 그대로 유지하고, 나머지 "GIA 운영" 계열 화면들은 shell-page-bg로 사이드바와 같은
// 테마 배경을 따라가게 했습니다("각 테마에 맞게 페이지 배경 통일" 요청). shell-content
// 클래스는 globals.css에서 다크/GIA 테마일 때 이 화면들 안의 text-slate-*/bg-white 같은
// 라이트 전용 클래스를 가독성 있게 다시 칠하는 스코프로 씁니다.
...
export default function MainArea({ children }: { children: React.ReactNode }) {
  ...
  if (isStaffManual) {
    return <main className="shell-content shell-page-bg flex-1 overflow-hidden p-4 sm:p-6">{children}</main>;
  }

  if (isBoundedList) {
    return (
      <main className={"flex-1 overflow-hidden p-4 sm:p-6 " + (isWeekly ? "bg-wr-bg" : "shell-content shell-page-bg")}>
        {children}
      </main>
    );
  }
  ...
  return (
    <main className={"flex-1 overflow-x-hidden p-4 sm:p-8 " + (isWeekly ? "bg-wr-bg" : "shell-content shell-page-bg")}>
      {children}
    </main>
  );
}
