-- ═══════════════════════════════════════════════════════════════════════════
-- 수납 · 단가 이력 · 인보이스 취소 사유 — Supabase SQL Editor 붙여넣기용
--
--   supabase/migrations/20260901180000_payments.sql
--
-- 앞서 실행한 20260901160000_fee_items_invoices.sql 다음에 걸어야 합니다(invoices를 참조).
-- 이걸 실행해야 재무 메뉴의 [수납] 탭이 열립니다.
-- ═══════════════════════════════════════════════════════════════════════════
...
values ('20260901180000', 'payments')
on conflict (version) do nothing;

commit;

-- 확인 — 세 줄 모두 true 여야 합니다.
select '수납(payments)' as 항목, to_regclass('public.payments') is not null as 있음
union all select '단가 이력(fee_item_price_log)', to_regclass('public.fee_item_price_log') is not null
union all select '인보이스 취소 사유 칸', exists (
  select 1 from information_schema.columns
  where table_schema='public' and table_name='invoices' and column_name='cancel_reason'
);
