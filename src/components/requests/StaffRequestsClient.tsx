  ...
  return (
    <div className="mx-auto flex h-full max-w-3xl flex-col overflow-hidden">
      <div className="shrink-0">
        ...
        {/* 카테고리 관리는 관리자만이 아니라 행정직원까지(교사 제외 전 권한) 가능합니다(요청:
            "카테고리 관리는 교사 이외의 권한들이 전부 할 수 있게 해줘"). isManager이 바로 그
            기준(isStaffOrAboveUser)입니다. */}
        {isManager && <CategoryManager categories={categories} onCategoriesChange={setCategories} />}
