...
- **제안함 "통합승인" 버튼 명칭 변경**: 학부모용·실무자용이 함께 묶여 나온 제안을 승인할 때
  버튼 문구가 "승인 (둘 다)"였는데, 뜻이 더 명확한 "통합승인"으로 바꿨습니다(단일 문서 제안은
  그대로 "승인").
- **"다시 분석하기" 기능 추가**: 지금까지는 사건·회의를 한 번 AI로 분석하면(제안함에 이미 들어간
  뒤에는) 다시 분석할 방법이 없었습니다. 이제 사건기록·회의기록 수정 화면과 제안함 카드 각각에
  "🔄 다시 분석" 버튼이 생겨서, 필요할 때 원하는 기록만 골라 다시 분석을 요청할 수 있습니다.
  다시 분석하면 그 기록에서 나온 기존 검토대기 제안은 새 분석 결과로 대체됩니다(이미 승인/보류로
  넘어간 이력은 그대로 보존됩니다).
- **중복 분석 방지 유지**: 자동/일괄 분석은 지금처럼 고유번호 기준으로 한 번만 처리되고, 이미
  분석된 기록을 다시 처리하려면 반드시 "다시 분석하기" 버튼을 직접 눌러야만 재분석이 일어납니다
  (실수로 같은 기록이 중복 분석되는 일이 없습니다).
- **제안함에서 원본 기록 재분석 가능**: 제안함(검토대기) 화면에서도 사건·행사·회의 유형의 카드라면
  바로 "🔄 다시 분석" 버튼으로 원본 기록을 다시 분석할 수 있습니다(원본 화면으로 돌아가지 않아도 됨).

## v0.74.0 - 2026-08-05 (staging)

- **운영계획안·매뉴얼 고정 항목 체계 도입**: 지금까지는 AI가 사건/회의를 분류할 때마다 항목
  이름을 그때그때 자유롭게 지어내서, 같은 주제인데 이름만 다른 항목이 계속 늘어날 수 있었습니다.
  이제 GIA시스템(학교 시스템 구비 현황)을 참고해 학부모님께 안내할 운영계획안 항목 20개를,
  다른 학교·국제학교의 컴플레인 대응 사례와 규정을 참고해 실무자 매뉴얼 항목 22개를 미리
  정리했습니다. AI 분류(신규 스캔·AI 매뉴얼 작성)는 이제 이 고정 목록 중에서만 골라야 합니다.
- **[정책 항목 관리] 화면 신설**: 학교 문서함 메뉴 안에 새 화면을 추가해, 관리자·행정직원이
  운영계획안/매뉴얼 항목의 이름·설명·보유상태를 직접 추가·수정·삭제할 수 있습니다.
- **GIA시스템 편집 권한 확장**: 지금까지 관리자만 GIA시스템 화면을 보고 고칠 수 있었는데,
  이제 행정직원도 볼 수 있고 항목명/설명을 직접 수정할 수 있습니다(요청: "관리자·행정직원까지"
  편집 가능하도록). 메뉴 위치도 관리자 전용 카테고리에서 학교 문서함 부메뉴로 옮겨 행정직원도
  찾아갈 수 있게 했습니다.
- **사건/회의 기록에 항목 선택 드롭다운 추가**: 사건기록·회의기록 작성/수정 화면에 "매뉴얼
  항목(실무자용)"과 "운영계획안 항목(학부모용)" 드롭다운을 추가해, 작성자가 직접 위 고정
  목록에서 골라 태그할 수 있습니다(비워두면 AI가 스캔할 때 자동으로 채웁니다).
- **기존 기록 소급 태깅**: [정책 항목 관리] 화면에 "🔄 기존 기록 소급 태깅" 버튼을 추가해,
  새 항목 체계 도입 전에 이미 저장된 사건/회의도 AI가 훑어서 고정 목록 중 해당하는 항목으로
  자동 태깅할 수 있습니다. 절차 문구를 새로 짓지 않고 항목명만 고르는 가벼운 작업이라 저렴한
  모델(Haiku)로 처리해 과금을 최소화했습니다.

## v0.73.1 - 2026-08-05 (staging)

- **출석부 5열 그리드**: 학생 목록을 화면 넓이에 맞춰 최대 5열 그리드로 배치했습니다(요청:
  "출석부 공간 넓으니까 페이지 다섯열로 나눠서"). 반은 기존처럼 학년·반 이름 순으로 정렬되고,
  결석/조퇴로 보호자 연락이 필요한 학생 카드는 연락처가 잘리지 않도록 그 줄 전체 폭을 씁니다.
- **출석부 메뉴 임시 비공개**: "일단 지금 출석부를 쓸건 아니니까"라는 요청에 따라 사이드바
  메뉴에서 출석부 항목을 뺐습니다. 화면과 기능은 그대로 남아있어 나중에 바로 다시 켤 수
  있습니다.

## v0.73.0 - 2026-08-05 (staging)

- **개발자 권한 비공개 강화**: 내 계정 설정, 학생 정보 조회, 관리자 대시보드, 데이터 백업,
  문의및 건의사항 화면에서 "개발자"라는 단어가 직접 노출되던 안내문구를 모두 "관리자/행정직원"
  또는 "담당자"로 바꿨습니다. 개발자 권한은 강경원 본인만 갖는 별도 등급이라, 다른 직원은 그런
  등급이 존재한다는 사실 자체를 알 수 없도록 UI 문구에서 완전히 뺐습니다(개발자 본인만 보는
  화면의 "개발자" 배지는 그대로 유지).
- **업무 삭제 버그 수정**: 업무를 삭제해도 등록자·담당자·관리자 화면에는 계속 목록에 남아있던
  문제를 고쳤습니다. 원인은 휴지통 조회용으로 추가했던 RLS 정책이 기존 조회 정책과 OR로
  합쳐지면서, 삭제 직후에도 그 정책을 통해 다시 보이던 것이었습니다. 업무보드·홈·알림·관리자
  대시보드 쿼리에 삭제 여부를 명시적으로 걸러 근본 원인을 고쳤습니다.
- **업무 휴지통 영구삭제/비우기**: 휴지통에 "🗑️ 영구삭제"(개별)와 "🗑️ 휴지통 비우기"(일괄)
  버튼을 추가했습니다. 등록자 본인이거나 관리자면 7일을 기다리지 않고 바로 완전히 삭제할 수
  있습니다. 기존에 모든 직원이 아무 업무나 하드 삭제할 수 있었던 과도하게 넓은 DB 권한도 함께
  정리해, 이제는 등록자 본인/관리자만 실제로 삭제할 수 있습니다.
- **행정요청 완료 처리자 표시**: 행정요청이 완료되면 누가 처리했는지(이름)가 요청 카드에 함께
  표시됩니다. 업무보드에서 담당자가 완료로 옮긴 경우와 행정요청 화면에서 직접 완료 처리한 경우
  모두 동일하게 기록되어, 교사가 자신이 등록한 요청의 처리자를 바로 확인할 수 있습니다.
- **학생 출석부 신설**: 담임교사가 매일 학생별 출결(출석/지각/결석/조퇴/기타)을 실시간으로
  체크하고, 행정직원·관리자 등 다른 직원도 같은 화면을 실시간으로 보면서 결석·조퇴한 학생의
  보호자에게 바로 전화/이메일로 연락하고 연락 완료 여부를 기록할 수 있는 "🗒️ 출석부" 메뉴를
  새로 만들었습니다. 교사는 자기 담임/부담임 반만, 그 외 직원은 전체 반을 볼 수 있습니다.
- **Toddle/ClassDojo 연동 조사**: 정규학기(Toddle)·계절학기(ClassDojo)에서 쓰는 학부모 소통
  앱과 gia-ops 앱을 자동 연동할 방법을 조사했습니다. 두 서비스 모두 현재 외부 개발자가 쓸 수
  있는 공개 API/웹훅이 없어(Toddle은 SIS→Toddle 방향의 출결 동기화만 지원하고 반대 방향
  내보내기는 없음, ClassDojo는 API 자체가 "얼리 액세스 신청" 단계) 자동 연동은 아직 불가능하며,
  대신 새로 만든 gia-ops 출석부를 단일 기준으로 쓰고 보호자 메시지 확인 후 수동으로 연락완료를
  체크하는 방식을 권장합니다(자세한 내용은 대화 참고).

## v0.72.0 - 2026-08-05 (staging)

- **업무 코멘트/실시간 로그 분리**: 업무 상세창에서 코멘트를 남기면 상태변경·업무확인 등
  시스템 로그가 코멘트창에도 함께 섞여 보이던 문제를 고쳤습니다. 이제 코멘트창에는 사람이
  직접 남긴 코멘트만 보이고, 상태변경/업무확인/코멘트 등록 알림 같은 시스템 로그는
  업무상황판 옆 "🔔 실시간 로그"에서만 보입니다.
- **GIA시스템 AI 제안 범위 제한**: "✨ AI로 추가/세분화 체크"(기존 "AI로 벤치마킹 제안받기")는
  이제 기존에 정리해둔 항목을 절대 고치거나 지우지 않고, ①GIA에 아직 없는 시스템을 새로
  추가하거나 ②이미 있는 항목 중 너무 뭉뚱그려진 것을 더 구체적인 하위 항목으로 쪼갤 수
  있는지만 체크해서 제안하도록 프롬프트와 저장 로직을 다시 짰습니다. 세분화 제안에는 어떤
  기존 항목을 세분화한 것인지 함께 표시됩니다.
- **GIA시스템 드롭다운 구조**: 대분류 안의 중분류 묶음이 전부 펼쳐진 채로 나열되던 것을,
  기본은 접혀있고 눌러야 펼쳐지는 드롭다운으로 바꿨습니다. 접힌 상태에서도 보유/부분보유/
  미보유 건수를 바로 볼 수 있습니다.
- **학생 통합 조회 재확인**: "학생 정보 조회" 메뉴(관리자·행정직원)에서 이름/학번으로 검색해
  들어가면 학적사항·학적이력·사건기록·주간 학생 관찰기록·업무/채팅 언급까지 한 화면에서
  보이는 기존 기능을 재확인했습니다(신규 구현 아님).

## v0.71.0 - 2026-08-05 (staging)

- **사건기록 채택·발행 통합**: 제안함에서 학부모용/실무자용을 따로 체크해 채택하던 방식을
  없애고, 승인·보류·삭제가 항상 둘을 함께 처리하도록 통합했습니다(운영계획안 내용은 자동으로
  실무자매뉴얼에도 들어가야 하므로). 대신 채택예정(발행) 화면에서 "통합발행"(운영계획안+
  실무자매뉴얼 함께 발행)과 "실무자발행"(실무자매뉴얼에만 발행 - 운영계획안에 넣기엔 사소한
  사건용) 두 가지로 나눴습니다.
- **채택 문구 톤 개편**: 운영계획안(학부모용)은 정중하고 친절한 안내 톤으로, 실무자매뉴얼
  (실무자용)은 명확한 지시 톤으로 다듬되, 둘 다 법령 조문처럼 명시적이고 정확한 표현을
  유지하면서 딱딱한 "~다"체 대신 정중한 "~하는 것으로 한다." 체로 종결하도록 AI 프롬프트를
  손봤습니다. 실무자용에도 학부모용과 동일하게 채택 시점에 한 번 더 다듬는 단계를 추가했습니다.
- **행정요청 메뉴 개편**: 행정요청 메뉴는 이제 교사에게만 보이고, 관리자/행정직원은 업무 탭의
  업무상황판에서 미처리 행정요청 건수를 바로 확인합니다(등록 시 자동으로 업무보드에도 올라가고
  처리 상태가 자동 동기화되므로 별도 메뉴가 필요 없습니다). 업무상황판을 좌우로 나눠 왼쪽엔
  기존 업무 상황, 오른쪽엔 행정요청 건수 + 톱니바퀴 아이콘(카테고리 관리)을 배치했습니다.
  카테고리 관리 권한도 관리자 전용에서 관리자·행정직원(교사 제외 전 권한)으로 넓혔습니다.

## v0.70.0 - 2026-08-05 (staging)

- **관리자 통합 대시보드 복구**: 반복 사건 유형, 학생 랭킹, 월별 운영 추이, 부서별 업무 완료율
  같은 관리자 전용 운영 분석을 별도의 "관리자 통합 대시보드"(관리자 메뉴 → 통합 대시보드)로
  다시 분리했습니다. 한때 학교 관리 대시보드(/school) 안으로 합쳤었는데, 로스터 화면 맨 아래에
  묻혀 찾기 어렵다는 문제가 있어 독립된 화면으로 되돌렸습니다. 관리자(개발자 포함)에게만
  보이고, 학교 관리 대시보드에서 바로 이동하는 링크도 추가했습니다.
- **채택예정 화면 개편**: 제안함과 같은 방식으로 카테고리 탭(사건/행사/회의/AI매뉴얼/예상
  문의/GIA시스템), 원본 사건 개요 표시, 학부모용·실무자용 카드 통합을 채택예정 화면에도
  적용했습니다. 예전에는 아코디언 목록만 있어 제안함이 개편된 뒤에도 채택예정만 예전 모습
  그대로였는데, 이제 두 화면의 보는 방식이 일관됩니다.
- **GIA시스템 항목 전면 재설계**: 기존 6개 항목(구비서류/내규/계약서/학생관리/교사관리/교직원관리)을
  지우고, 다른 공립·사립·국제학교가 일반적으로 갖추는 시스템을 참고해 대분류(재정/인사·교직원/
  학사/운영/시설·안전/입학·홍보/행정·문서/정보보안·법무) → 중분류 → 세부 항목의 3단계로 44개
  항목을 새로 구성했습니다. 각 항목에 "📁 서류함에 만들기" 버튼을 추가해, 필요한 서류가 있으면
  같은 대분류/중분류 분류가 그대로 적용된 서류를 서류함에 바로 만들 수 있습니다.
- **AI 프롬프트 공유 캐시 블록 통합**: 기관 소개문과 참고 법령 목록을 쓰는 6개 AI 프롬프트(사건
  분류, AI매뉴얼 초안 분류, 서류 추천, 서류 초안, 예상 문의/컴플레인, 채택예정 AI 검증)가 각자
  전체 시스템 프롬프트를 하나의 문자열로 캐싱하던 것을, 공용 부분(SHARED_CACHE_CONTEXT)과
  라우트별 부분으로 나눠 별도의 캐시 블록으로 보내도록 바꿨습니다. Claude API의 프롬프트
  캐싱은 완전히 동일한 접두사에만 적중하므로, 이제 서로 다른 AI 기능을 연달아 호출해도 공용
  부분은 캐시가 재사용되어 입력 토큰 비용이 줄어듭니다.

## v0.69.1 - 2026-08-05 (staging)

- **옵션 선택 시 최종 채택 내용 자동 기입**: 제안함에서 보완방안/학부모 안내멘트/학생 교육방법
  옵션 앞에 동그라미(라디오) 선택을 추가했습니다. 하나를 고르면 그 내용이 그대로 "최종 채택
  내용" 박스에 채워져서, 바로 다듬어 채택할 수 있습니다(체크박스는 다중 선택이 가능해 보여
  혼동을 줄 수 있어 한 번에 하나만 고르는 동그라미 방식으로 바꿨습니다).
- **재발방지 옵션을 실제 절차로 강화**: AI가 제안하는 보완/재발방지 방안이 추상적인 다짐이
  아니라, 상황 발생 → 담당자의 즉시 조치 → 학부모에게 안내할 선택지 → 선택지별 후속 조치 →
  단계별 학부모 고지 시점 → 최종 인계 확인까지, 실제로 그대로 따를 수 있는 순서와 책임
  소재가 분명한 절차로 작성되도록 프롬프트를 강화했습니다. 이 절차를 운영계획안과
  실무자매뉴얼에 항목별로 반영하면, 학부모와 실무자 모두가 정해진 절차를 미리 알고 나중에
  그대로 지켜졌는지 확인할 수 있습니다.
- **즉시 채택 전 확인창**: 제안함 목록에서 학부모용/실무자용 체크박스를 눌러 바로 채택예정으로
  보낼 때, 사건명과 최종 채택 내용을 다시 보여주고 확인을 받은 뒤에만 진행합니다.
- **채택예정 되돌리기**: 채택예정 화면에 "↩️ 되돌리기" 버튼을 추가했습니다. 실수로 승인했거나
  다시 검토하고 싶은 항목을 제안함(검토대기)으로 되돌릴 수 있습니다(단, 이미 발행되어 매뉴얼에
  반영된 항목은 되돌릴 수 없습니다).

## v0.69.0 - 2026-08-05 (staging)

지난 통합 개선 제안서(v0.68.0)의 8개 항목을 전부 반영하고, AI 비용을 더 아끼는 방향으로
함께 설계했습니다.

- **① 원본 사건/회의 역참조 링크**: 매뉴얼 항목이 어떤 사건/행사/회의/GIA시스템 기록에서
  왔는지 카드 하단에 칩으로 표시하고, 원본 목록 화면으로 바로 이동할 수 있습니다. 발행 후에도
  이 연결이 유지됩니다.
- **② 반복 사건 패턴 ↔ 매뉴얼 연결**: 홈 화면의 "최근 90일 반복 사건" 판정(AI 호출 없이 순수
  집계, 비용 0)을 매뉴얼 화면에도 그대로 적용해, 관련 항목 카드에 "🔥 반복 사건 N건" 배지를
  띄웁니다.
- **③ GIA시스템 자동 매칭**: 매뉴얼이 발행될 때, "미보유" 상태인 GIA시스템 항목의 이름이
  발행 문구에 그대로 언급되어 있으면(AI 호출 없이 단순 포함 여부만 확인) 자동으로 연결해두고,
  GIA시스템 화면에서 안내 문구로 보여줍니다. 실제 "보유" 전환은 사람이 확인 후 직접 합니다.
- **④ 정책영역(도메인) 분류**: 사건/회의/AI매뉴얼 작성 시 AI가 항목을 8개 정책영역(학사·
  안전/보건·시설·인사·재무/환불·개인정보·민원대응·행정일반) 중 하나로 함께 분류합니다(기존
  분류 호출에 슬쩍 얹어서 별도 AI 호출은 늘리지 않았습니다). 매뉴얼 화면에서 정책영역별로
  필터링할 수 있습니다.
- **⑤ 매뉴얼 변경 이력 관리**: 항목이 수정될 때마다 이전 내용이 자동으로 이력에 남고,
  "변경 이력" 버튼으로 과거 버전을 언제든 확인할 수 있습니다.
- **⑥ AI매뉴얼 작성 시 유사 기록 추천**: 새 초안을 작성하면, AI 호출 없이 키워드 겹침만으로
  과거 사건/회의/이미 발행된 매뉴얼 중 관련 있어 보이는 기록을 함께 보여줍니다.
- **⑦ 발행 전 학부모 관점 체크리스트**: 학부모용 항목을 AI 검증할 때는 "학부모가 이 문구만
  읽고 오해 없이 이해할 수 있는가"를 최우선 관점으로 점검하도록 프롬프트를 강화하고, 버튼
  라벨도 "학부모 관점 AI 검증"으로 구분했습니다.
- **⑧ 매뉴얼 정기 리뷰 사이클**: 매주 월요일, 6개월 이상 수정되지 않은 항목과 최근 90일 내
  관련 사건이 5건 이상 급증한 항목을 자동으로 찾아 매뉴얼 화면 상단에 배너로 알립니다(AI
  호출 없이 조건 판정만). 관리자가 "확인 완료"로 해소합니다.
- **AI 과금 최소화**: 학부모용 승인 시 톤을 다듬는 AI 호출을 Sonnet에서 Haiku로 낮췄습니다
  (이미 승인된 문구를 존댓말로만 바꾸는 단순 작업이라 저렴한 모델로 충분합니다). 위 8개
  항목 중 ②③⑥⑧은 처음부터 AI를 전혀 쓰지 않는 방식(순수 집계/키워드 매칭)으로 설계해서,
  기능을 늘리면서도 AI 호출 횟수는 늘어나지 않았습니다.

## v0.68.0 - 2026-08-05 (staging)

- **제안 카드에 사건 개요 노출**: 사건/행사/회의/AI매뉴얼 초안에서 나온 제안을 펼치면, 그
  제안이 어떤 원본 기록에서 나온 건지 제목·내용 요약을 함께 보여줍니다.
- **체크박스로 즉시 채택**: 카드를 펼치지 않아도 목록에서 체크 한 번으로 바로 승인해
  채택예정으로 옮길 수 있습니다.
- **"다음 옵션" 반복 문구 제거**: 보완방안/학부모 안내멘트/학생교육 옵션이 화면에 구분자
  글자 없이 번호가 매겨진 카드 형태로 깔끔하게 표시됩니다.
- **제안서 형식으로 가독성 개선**: 사건 개요 / 채택 내용 / 제안 옵션 / 법령 근거를 구분선으로
  나눠 문서처럼 읽기 쉽게 재구성했습니다.
- **학부모용·실무자용 통합 카드**: 같은 사건에서 나온 학부모용/실무자용 제안 두 건이 카드
  하나로 합쳐지고, 안에서 탭으로 전환해서 봅니다(전에는 같은 사건이 카드 두 개로 따로
  보였습니다).
- **학부모용 승인 시 톤 다듬기 확대**: 전에는 예상 문의(컴플레인) 제안만 승인 시 AI가 톤을
  다듬었는데, 이제 모든 학부모용 승인 건이 정중하고 친절한 문구로 한 번 더 다듬어집니다.
- **AI가 기존 항목을 우선 재사용**: 사건/행사/회의/AI매뉴얼 제안을 만들 때, 운영계획안·
  실무자매뉴얼에 이미 있는 항목 목록을 AI에게 실제로 전달해서, 비슷한 내용이 항목명만
  다르게 계속 새로 생기는 문제를 줄였습니다.
- **사건·회의·AI매뉴얼 통합 개선 제안서**: 앞으로 더 유용하게 연결할 수 있는 방안(발행 후
  원본 연결 유지, GIA시스템 자동 매칭, 매뉴얼 변경 이력 관리 등)을 정리한 제안서를
  작성해 전달했습니다.

## v0.67.0 - 2026-08-05 (staging)

- **행정요청 카테고리를 관리자가 직접 관리**: 사물함파손/물품구입 같은 요청 분류를 관리자가
  화면에서 새로 추가하거나 숨길 수 있습니다(운영관리 화면이 아니라 행정요청 화면 안에
  "카테고리 관리" 패널로 있습니다).
- **행정요청 등록 시 초등부 업무보드에 자동 등록**: 교사가 요청을 등록하면 초등부 전체
  업무창에도 업무가 함께 생성됩니다. 나중에 담당 행정직원별로 자동 배정하는 기능을 붙일
  기반을 마련해뒀습니다.
  - 업무보드에서 담당자가 "확인" 체크를 하면 요청 목록의 빨간불이 초록불로 자동으로
    바뀌고, 업무를 완료로 옮기면 요청도 자동으로 완료 처리됩니다.
  - 교사 화면은 "내가 등록한 요청"과 그 아래 "완료된 요청" 두 목록으로 나눠서, 진행 중인
    요청과 끝난 요청을 한눈에 구분할 수 있습니다.
- **행정요청 코멘트 기능**: 요청 하나하나에 코멘트를 주고받을 수 있고, 새 코멘트는 실시간으로
  반영됩니다.
- **한/영 자동 번역**: 요청 제목·내용과 코멘트를 한국어로 적든 영어로 적든 자동으로 번역되어
  함께 표시됩니다(원어민 교사가 많은 점을 고려했습니다).
- **행정요청 화면 전체 영어 병기**: 버튼·라벨·안내 문구에 영어를 함께 표기했습니다.

## v0.66.1 - 2026-08-05 (staging)

- **교사 메뉴 영어 병기**: 교사 계정에 원어민 교사가 있을 수 있어, 사이드바·모바일 메뉴의
  "행정요청"에 영어("Staff Requests")를 함께 표기했습니다. 이미 영어가 붙어 있던 "내
  담임반"/"내 담당과목"과 함께 교사가 보는 메뉴 전체에 영어가 병기됩니다.
- **메뉴 줄바꿈 정리**: 한글/영어 라벨이 길어져도 어색하게 줄이 꺾이지 않고 한 줄로
  깔끔하게 잘리도록(말줄임표) 사이드바·모바일 메뉴 스타일을 정리했습니다.

## v0.66.0 - 2026-08-05 (staging)

- **교사 메뉴에서 학사일정 제거**: 교사 계정에는 더 이상 학사일정 메뉴가 보이지 않습니다.
- **행정요청 메뉴 신설**: 교사가 사물함 파손·물품 구입·아픈 학생 인계·출결 상황 문의처럼
  행정직원에게 부탁할 일을 등록하면, 행정직원/관리자가 접수대기 → 처리중 → 완료로
  상태를 바꾸고 처리 메모를 남길 수 있습니다. 교사는 학사일정 자리에서, 다른 직원은
  사이드바 상단에서 바로 들어갈 수 있습니다.
- **권한 미리보기 전환 버그 수정**: 개발자가 "교사"로 미리보기 전환한 뒤 다른 권한으로
  다시 바꿀 수 없던 문제를 고쳤습니다. 실제 교사 계정이 API를 호출할 때도 같은 문제로
  일부 기능이 조용히 막혀 있었는데 함께 해결됐습니다.
- **브라우저 탭 아이콘 교체**: Vercel 기본 아이콘 대신 로그인 화면의 GIA 크레스트 로고가
  탭 아이콘으로 표시됩니다.

## v0.65.0 - 2026-08-05 (staging)

- **운영관리 부메뉴 정리**: "매뉴얼" 항목을 뺐습니다. 학교 문서함에 이미 매뉴얼로 가는
  입구가 있어 같은 화면으로 가는 길이 두 군데로 겹쳤습니다.
- **화면 전체 크래시 방지 강화**: 검토대기 배지처럼 부가적인 정보 조회가 실패해도
  로그인·사이드바를 포함한 화면 전체가 함께 죽지 않도록 서로 독립시켰습니다. 화면이
  깨졌을 때 자동으로 오류 기록이 남아, 신고하지 않아도 개발자가 바로 확인할 수 있습니다.
- **로그성 데이터 조회 속도 개선**: 계속 쌓이기만 하는 오류·AI 사용 기록, 미해결 문의
  조회에 인덱스를 추가했습니다.
- **동시 사용 안전장치 확대**: 학기 전환과 채택예정 발행을 여러 명이 거의 동시에 눌러도
  데이터가 꼬이지 않도록 서버에서 한 번에 처리하게 바꿨습니다. 매뉴얼/학기 항목을 다른
  사람이 지금 편집 중이면 화면에 바로 표시됩니다.
- **개발자 대시보드에 시스템 상태 카드 추가**: 최근 24시간 오류 급증 여부와 마지막 백업
  시각을 한눈에 볼 수 있습니다.
- **자동 일일 백업**: 매일 한 번 핵심 데이터를 자동으로 백업해두어, 수동 백업을 깜빡한
  날에도 최소한의 안전망이 있습니다.
