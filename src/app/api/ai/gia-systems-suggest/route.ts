...
아래는 GIA가 "이미 갖췄거나 이미 검토한" 항목 목록입니다(대분류/중분류/이름 순) - 이미 있는 항목은
다시 제안하지 말고, 정말 GIA에 없는(또는 부족한) 것 위주로 3~6개만 제안하세요:
{{EXISTING}}
...
export async function POST() {
  ...
  try {
    ...
    const result = (await callClaudeJsonWithWebSearch(
      ...
      suggestions?: { major: string; category: string; name: string; description: string; benchmark_school?: string }[];
    };
