...
export async function POST(req: Request) {
  ...
  // 길이 제한은 두지 않습니다(요청: "공용계정 비밀번호 제한없게 해줘"). 도서관 노트북처럼
  // 여러 사람이 하루에도 몇 번씩 입력하는 계정은 짧고 외우기 쉬운 편이 실제로 더 안전합니다 -
  // 길고 복잡하게 강제하면 결국 모니터에 포스트잇으로 붙게 되기 때문입니다.
  //
  // 다만 빈 값은 로그인 자체가 불가능해지므로 막습니다. 여기서 통과하더라도 Supabase 프로젝트
  // 설정에 정해진 최소 길이(기본 6자)보다 짧으면 Supabase가 거절하고, 그 메시지가 아래에서
  // 그대로 화면에 표시됩니다.
  if (!password) {
