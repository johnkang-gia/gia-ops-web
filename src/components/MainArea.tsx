...
export default function MainArea({ children }: { children: React.ReactNode }) {
  ...
  if (WIDE_TABLE_PATHS.some((p) => pathname === p)) {
    // **좌우 여백을 두지 않습니다.** 열이 열 개를 넘는 표라, 여백으로 내준 만큼이 그대로
    // 잘리는 칸이 됩니다 - 여백은 넓은데 오른쪽 끝의 「청구액·청구서」는 안 보이는 상태가
    // 됐습니다. 글에 필요한 안쪽 여백은 화면이 스스로 둡니다.
    return <main className="shell-content shell-content-bg min-w-0 flex-1 overflow-x-hidden px-0 py-3">{children}</main>;
  }
