-- 요금제와 할인 (인보이스 시스템 2단계)
--
-- 담당자:
--   "할인 적용이 몇 개 더 있는데 그건 부이사장님만 아셔."
--   "할인률과 항목들을 자유롭게 설정할 수 있게 (재무 권한이 있는 사람이) 만들어줘."
--   "형제할인 같은 부분 원래는 있는데 없애신다고 하셨거든. 그래서 할인 항목의 경우
--    자유롭게 만들었다가 없앴다가 될 수 있게."
--
-- ── 왜 금액표가 아니라 '요금제 + 옵션'인가 ──────────────────────────────
--
...
-- ── 학년도가 연도를 걸치는 문제 ─────────────────────────────────────────
--
-- 담당자: "지금 학기가 26-27년도 1학기야. 학년당 3학기는 맞는데 그게 연도를 걸쳐 있어."
--
...
create table if not exists public.fee_plans (
  ...
  -- 요금이 오르면 이전 요금제를 끄고 새로 만듭니다. 금액을 덮어쓰면 지난 청구서가
...
comment on table public.fee_plans is
  '요금제. 기준 금액 1회분만 담고, 묶음·할인은 fee_payment_options가 정합니다.';
...
-- ═══════════════════════════════════════════════════════════════════════
--
-- 담당자: "형제할인 같은 부분 원래는 있는데 없애신다고 하셨거든."
--
-- 그래서 **지우지 않고 끕니다(active=false).** 이게 핵심입니다. 지워버리면 작년 청구서가
-- 왜 그 금액이었는지 설명할 수 없게 됩니다. "지금 쓰는 할인"과 "그때 썼던 할인"은 다른
-- 물음이고, 둘 다 답할 수 있어야 합니다.
--
-- 그리고 "부이사장님만 아시는 할인"이 있다고 하셨습니다. 그래서 이 표 전체가 재무 권한
