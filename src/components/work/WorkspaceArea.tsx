...
export default function WorkspaceArea({
  ...
  // 왼쪽: 업무 상황판(숫자 배지, 작게) + 채팅(크게) - 상황판:채팅 = 대략 3:7.
  // 오른쪽: 내 업무목록(위젯) + 칸반보드(진행대기/진행중/보류이슈/완료).
  const [leftWidth, setLeftWidth] = useState(45);
  const [leftTopHeight, setLeftTopHeight] = useState(30);
  ...
  function startRowResize(setter: (v: number) => void, current: number) {
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex flex-col overflow-hidden" style={{ width: `${leftWidth}%` }}>
        ...
        <div
          onMouseDown={startRowResize(setLeftTopHeight, leftTopHeight)}
