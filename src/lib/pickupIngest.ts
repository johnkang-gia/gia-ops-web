...
- "오늘 치과 때문에 10시반부터 수업을 빠졌어요. 치료 마쳤고 학교 앞인데 마취 풀리면 들어가려고요"
  → **픽업 아님.** 이미 지나간 일이고, 무엇보다 **아이가 학교로 돌아옵니다.**
    "빠졌어요 · 데려갔어요 · 늦었어요"는 지나간 일을 알리는 말이지 앞으로의 하원 계획이
    아닙니다. 이런 건 "문의"(늦은 등교·외출 알림)이고 confidence는 0.3 이하입니다.

**돌아온다는 말이 있으면 픽업이 아닙니다.**
"들어갈게요 · 다시 등교 · 오후에 보낼게요 · 이따 데려다 줄게요"가 있으면, 앞에 무슨 말이
있어도 하원 픽업이 아닙니다. 픽업은 그 아이가 **오늘 학교에서 나가서 안 돌아오는** 일입니다.

**지나간 일과 앞으로의 일을 가르세요.**
"~했어요 · ~였어요 · 빠졌어요 · 데려갔어요"는 이미 일어난 일입니다. 하원 픽업은 앞으로의
계획이라 "~할게요 · ~하겠습니다 · ~갑니다"로 적힙니다. 과거형만 있으면 픽업이 아닙니다.
...
export async function applyPickup(
  ...
  for (const assignmentId of ids) {
    ...
    // **이미 픽업인 줄은 손대지 않습니다.**
    //
    // 예전에는 상태가 이미 「픽업」이어도 다시 쓰고 활동 기록을 남겼습니다. 바뀐 것이 없는데
    // 기록에는 「픽업 → 픽업」이 남고, 표가 바뀌었으니 번호 문지기(bump_ops)가 올라가
    // **중앙 대시보드와 학생 하루 보드가 그 아이만 계속 새로 고쳐졌습니다.**
    //
    // 남가인이 그랬습니다. 배정이 네 줄이라 크론이 한 번 돌 때마다 「자동이 상태를
    // 바꿨습니다」가 네 줄씩 쌓였고, 화면에는 오류가 아니라 **방금 무슨 일이 또 있었던
    // 것처럼** 보였습니다. 기록을 읽는 사람은 실제 변경과 구별할 수 없습니다.
    //
    // 아무것도 안 바뀐 일은 기록이 아닙니다.
    if (existing && (existing.status as string | null) === "픽업") {
      continue;
    }
