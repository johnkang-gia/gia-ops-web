...
- **GIA 테마 본문 배경 밝게 조정**: 바로 전 업데이트(v0.61.0)에서 GIA 테마를 고르면 사건기록·
  ...
  맞아 읽기 힘들다는 피드백을 반영해 사이드바/헤더는 짙은 남색 그대로 두고, 본문 배경만
  골드 톤과 어울리는 밝은 아이보리/샴페인 그라디언트로 바꿨습니다. 검은 글자 그대로도 잘
...
- **페이지 배경도 테마 적용**: 지금까지는 사이드바/헤더 같은 공용 틀만 테마(라이트/다크/
  ...
  등 실제 화면 배경은 항상 밝은 회색 고정이었습니다. 이제 이 화면들의 배경도 선택한 테마를
  그대로 따라갑니다(다크 테마=짙은 남색, GIA 테마=남색 그라디언트, 리퀴드글라스=은은한
  하늘색 그라디언트). 업무 탭과 위클리 리포트는 원래도 각자 고유한 색(업무=블루 글래스,
  위클리=틸)을 쓰는 게 의도된 디자인이라 이번에도 그대로 뒀습니다.
- **테마별 글자색 가독성 보정**: 배경이 어두워지는 다크·GIA 테마에서, 화면 안의 글자색이
  기존처럼 어두운 톤 그대로면 안 보이는 문제가 있어 자동으로 밝은 톤으로 바뀌도록 손봤습니다.
  제목/본문 글자, 카드 배경, 테두리, 마우스 오버 색까지 다크 테마는 슬레이트 계열로, GIA
  테마는 크림/골드 계열로 맞춰 톤을 통일했습니다. 라이트·리퀴드글라스는 배경이 원래도 밝아서
...
- **버전 기록**: 사이드바 문의및건의사항 바로 아래에 현재 버전(v0.59.0)이 뜨고, 누르면
  ...
  이동합니다. `CHANGELOG.md` 내용을 그대로 불러와 보여주므로 앞으로 버전을 올릴 때마다
  이 파일에 기록하는 기존 습관만 유지하면 자동으로 반영됩니다.
- **테마 정교화**: 리퀴드글라스 테마는 사이드바 메뉴 버튼이 실제로 반투명+블러 유리질감으로
  뜨고, GIA 테마는 사이드바 배경이 로고 남색, 선택된 메뉴가 골드 톤으로 강조되도록
  다듬었습니다. 검색창/달력 위젯/로그아웃 버튼 등 셸 전체의 글자색도 테마에 맞춰 조정됩니다.
- **모션**: 사이드바 부메뉴(플라이아웃)와 검색 결과가 아래로 살짝 미끄러지며 나타나고,
  업무 알림 배지가 새로 생길 때 팝(pop)하듯 나타나며, 메뉴 버튼을 누르면 살짝 눌리는
...
- **프로필 이름 오른쪽(회색 사각 배지)**: 확인 여부와 무관하게 "내 업무" 총 개수를
  항상 보여줍니다.
- **즉시 반영 버그 수정**: "업무 확인" 체크박스를 누르면 Realtime 전파를 기다리지 않고
...
- **숫자**: 이제 "새로 생긴 업무 수"가 아니라 "지금 내 업무함에 있는 업무 개수"입니다(내
  ...
  아직 완료 안 된 것). 채팅 안읽음은 이 배지에서 뺐습니다(업무 개수만 표시).
...
- **메뉴 하이라이트 버그 수정**: "학교관리"를 누르면 "관리자"까지 같이 색이 바뀌고,
  "학교문서함"을 누르면 "학교관리"·"관리자"가 같이 바뀌고, "관리자"를 눌러도 "학교관리"가
  같이 바뀌던 문제를 고쳤습니다. 원인은 두 가지였습니다: (1) "관리자" 메뉴 안에 "학교
  관리"와 완전히 같은 주소(/school)로 가는 "학교 현황판" 항목이 중복으로 들어있었던 점,
  (2) 주소가 겹치는 메뉴(예: "/school"과 "/school/documents")를 단순 접두어로만 비교해서
  더 구체적인 페이지에 있어도 바깥 메뉴까지 함께 켜지던 점. 중복 항목을 정리하고, 지금
...
새 업무가 "내 업무목록"에 들어오면(내가 태그되었거나 [전체]로 등록된 경우) 확인창(alert)
없이 화면 오른쪽 아래에 잠시 알림이 떴다가 자동으로 사라지도록 했습니다.

- 이미 있던 "업무 상태 변경 토스트"와 같은 방식으로, 새 업무 등록 시에도 알림이 뜨도록
  확장했습니다. "OOO님이 새 업무 "제목"를 등록했어요" 형태로 6초간 표시되고, 클릭하면 바로
...
접속중인 사람이 누구인지 마우스로 확인할 수 있게 하고, 업무 상세창의 코멘트 영역을
코멘트/처리사항 둘로 나누고, 처리사항이 업무기록과 보고서(화면+PDF)까지 이어지도록
만들었습니다.

- **접속자 호버 툴팁**: 업무 탭 상단 "🟢 N명 접속중" 배지에 마우스를 올리면 지금 이
  페이지를 보고 있는 사람들의 이름이 뜹니다.
- **코멘트/처리사항 분할**: 업무를 클릭해 여는 상세창의 코멘트 영역을 위/아래로 나눴습니다.
  위쪽은 기존 코멘트 목록·입력창 그대로이고, 아래쪽은 새로 생긴 "📝 처리사항" 입력란으로,
  이 업무를 어떻게 처리·완료했는지를 남길 수 있습니다. 입력 후 다른 곳을 클릭하면
  자동 저장됩니다.
- **처리사항이 보관함·보고서까지 이어짐**: 처리사항은 업무 자체에 저장되는 값이라
  업무가 완료→보관함(업무기록)으로 넘어가도 그대로 남고, 업무기록 화면과 업무 보고서
  화면·PDF에서도 "처리사항/처리결과"로 함께 보입니다. 업무 보고서 PDF의 "비고" 열은
  "업무결과" 열로 바뀌어 처리사항을 보여줍니다(처리사항이 없으면 기존 설명을 대신 보여줍니다).
- **코멘트가 실시간 로그에도 기록됨**: 업무에 코멘트를 남기면 상단 실시간 로그에도
  "OOO님이 코멘트를 남겼습니다: ..." 한 줄로 함께 기록됩니다.
- **실시간 로그 5줄 제한**: 확인해보니 이 부분은 이전 작업에서 이미 반영되어 있었습니다
...
- **달력 클릭 → 학사일정 이동**: 날짜 칸이든 달력의 다른 빈 곳이든, 어디를 눌러도
  `/academic-calendar`(학사일정)로 이동합니다. 단, 월 이동 버튼(‹ ›)은 그대로 달만
  넘깁니다.
- **OS 캘린더 앱 열기 기능 제거**: 날짜를 누르면 맥 캘린더 앱(또는 .ics 다운로드)을
  여는 기존 기능이 실제로는 잘 뜨지 않는다는 제보를 받아 제거했습니다. 이 위젯은
  애초에 학사일정과 연동된 미리보기라, 학사일정 페이지로 바로 보내는 게 더 자연스럽고
  확실합니다.
...
- **화면을 막던 조회 축소(5개 → 3개)**: 로그인 확인과 배지 숫자만 화면 렌더링 전에
  ...
  없었습니다. 이 둘을 별도 조각으로 분리해 스트리밍(Suspense)으로 바꿨습니다 - 로고·
  검색창·메뉴는 즉시 뜨고, 학기 배지만 아주 살짝 늦게(수십~수백ms) 채워집니다.
- 메뉴/페이지별 데이터 조회 구조를 점검해 대부분 이미 병렬 조회로 잘 구성돼 있음을
  확인했습니다(사건기록/홈 등 - 이전 라운드에서 이미 최적화됨).
...
사이드바 메뉴를 `<Link>`에서 버튼+`router.push()`로 바꾸면서(마우스를 올리면 창 아래
상태표시줄에 링크 주소가 뜨는 걸 막기 위함) Next.js가 `<Link>`에 기본으로 붙여주던
"미리 불러오기(prefetch)" 기능까지 함께 사라진 것이었습니다 - 그래서 메뉴를 누르는
순간에야 서버에 화면 데이터를 요청하기 시작해 매번 느리게 느껴졌습니다.

- **메뉴 hover 시 미리 불러오기 복구**: 마우스를 메뉴 위에 올리는 순간(부메뉴가 펼쳐지는
  순간 포함) 그 메뉴가 갈 화면 데이터를 미리 받아두도록 했습니다. 실제로 클릭할 땐 이미
  데이터가 준비돼 있어 전환이 훨씬 빠릅니다. 모바일(터치)은 호버가 없어 화면에 메뉴가
  나타나는 즉시 전부 미리 받아둡니다.
- **최근 열어본 화면 재사용(30초)**: 같은 메뉴를 30초 안에 다시 열면 서버에 다시 묻지
...
중복 처리되지 않도록 손봤습니다. 지금까지는 두 사람이 동시에 누르면 같은 미분석 기록을
각자 읽어가서 AI를 두 번 호출하고 제안도 두 번 저장되는 경우가 있었습니다(에러는 안 나지만
제안이 중복 생성되고 AI 비용도 그만큼 더 나감).

- **원자적 선점(claim) 처리**: AI를 부르기 전에 먼저 "이 기록은 내가 처리한다"고 DB에
  원자적으로 표시합니다. 두 요청이 동시에 같은 기록을 가져와도 하나만 통과하고, 나머지는
  자동으로 건너뜁니다(사건/행사/회의 스캔 모두 적용). 처리 도중 오류가 나면 표시를 되돌려서
...
- **검색+달력 위젯 통합**: 따로 있던 통합 검색창과 축소 달력을 검색이 위, 달력이 아래에
  오는 하나의 상자로 합치고 전체 크기를 더 줄였습니다 - 그만큼 메뉴 목록이 보이는 영역이
  넓어집니다.
...
새 메뉴 "학사일정"을 추가했습니다. 학기 시작(또는 종료) 며칠 전에 무엇을 준비해야 하는지를
달력으로 한눈에 보고, 전 직원이 체크할 수 있는 화면입니다.

- **학사일정 달력 화면 (`/academic-calendar`, 신규)**: 이번 달 달력에 학사 준비 업무가
  날짜별로 표시됩니다(예: 학기 시작 14일 전 - 학생명단 확정, 7일 전 - 교실 물품 점검). 각
  항목은 체크박스로 완료 표시하고(처리자·시각 기록), 다음 담당자를 위한 메모도 남길 수
  있습니다. 관리자는 "반복 체크리스트 관리"에서 템플릿(제목·담당·기준일 며칠 전)을 등록·
  수정·삭제할 수 있고, 이렇게 등록해두면 학기가 바뀔 때마다 이 페이지를 여는 순간 그 학기
  기준 날짜로 자동 채워집니다(매번 새로 등록할 필요 없음). 반복 항목 외에 일회성 준비 업무도
  "+ 항목 추가"로 바로 등록할 수 있습니다.
- **사이드바 메인 달력과 연동**: 사이드바에 항상 떠 있는 달력(시계 위젯)에도 학사일정이 있는
  날짜에 작은 초록 점이 표시되고, 달력 아래 "📅 학사일정 보기" 링크로 바로 이동합니다. 교사
...
채팅을 계속 치면 채팅창이 아래로 늘어나면서 브라우저 페이지 전체가 스크롤돼, 사이드바
메뉴가 화면 위로 밀려 올라가버리는 문제를 고쳤습니다.

- **화면 레이아웃 높이 고정**: 사이드바+본문을 감싸는 바깥 틀이 "최소 높이"(min-height)로만
  잡혀 있어서, 채팅처럼 내용이 계속 늘어나는 화면에서는 그 틀 자체가 계속 커지며 전체
...
전체 메뉴 구조를 점검해 중복되는 화면을 통합하고, 검토가 필요한 항목을 사이드바에서 바로
알 수 있도록 배지를 추가했습니다.

- **검토 대기 배지**: 사이드바 운영 관리 메뉴에 제안함(검토대기)·채택예정(발행대기) 건수가
  빨간 숫자 배지로 표시됩니다. 메뉴를 열어보지 않아도 지금 처리할 게 몇 건인지 바로 보입니다.
- **대시보드 통합**: 예전에 따로 있던 "관리자 대시보드"(반복 사건·학생 랭킹·월별 추이·부서별
  업무 완료율 분석)를 "학교 관리 대시보드"(/school) 하나로 합쳤습니다. 관리자는 이제 반/과목/
...
학교관리 아래에 "학교 문서함" 메뉴를 새로 만들어, GIA에서 만들어지는 보고서·매뉴얼·운영계획안·
서류를 한곳에서 열람·검색·인쇄할 수 있도록 통합했습니다.

- **학교 문서함 (신규 메뉴)**: 사이드바에 🗄️ 학교 문서함을 새로 추가했습니다. 마우스를 올리면
  보고서(업무·회의) / 매뉴얼 / 운영계획안 / 서류함 네 개 항목이 부메뉴로 펼쳐집니다.
- **문서함 홈 (`/school/documents`)**: 매뉴얼·운영계획안·서류함에 등록된 문서를 최근 등록·수정
  순으로 모아 보여주고, 이름/카테고리로 바로 검색할 수 있습니다. 보고서·매뉴얼·운영계획안·
  서류함으로 바로 이동하는 카드도 함께 있습니다.
- **보고서 통합 화면 (`/school/documents/reports`)**: 업무 보고서와 회의 보고서를 탭 하나로
  오가며 볼 수 있습니다(기존 `/work/report`, `/meetings/report` 화면과 동일한 내용이며, 두
  메뉴는 그대로 유지됩니다).
- **매뉴얼/운영계획안 바로가기**: 매뉴얼 화면이 `?doc=` 링크로 들어오면 실무자매뉴얼/운영계획안
...
사이드바 메뉴에 마우스를 올리면 브라우저 창 아래쪽 상태표시줄에 계속 링크 주소가 떠서
거슬리던 문제를 고쳤습니다.
...
사이드바 부메뉴가 화면 아래로 잘리는 문제를 고치고, 업무·회의를 일간/주간/월간 보고서로
정리해 바로 인쇄(PDF)할 수 있는 화면을 새로 만들었습니다.

- **부메뉴 잘림 수정**: 사이드바 아래쪽 메뉴에 마우스를 올리면 뜨는 부메뉴가 화면 밑으로
  잘려나가 누르기 어려웠던 문제를 고쳤습니다. 이제 실제 부메뉴 높이를 재서, 화면 밖으로
  넘치면 자동으로 위로 올려 항상 전체가 보이도록 보정합니다.
- **업무 보고서 (신규, 🗂 업무 페이지 상단 아이콘 → 📊 업무 보고서, `/work/report`)**: 일간
  ·주간·월간 단위로 그 기간에 완료된 업무(언제·누가·무엇을)와 아직 진행 중이던 업무 현황을
  한 화면에 모아 보여줍니다. "🖨 인쇄/PDF" 버튼을 누르면 초등부 전체 업무 보고서가 새 탭에
  PDF로 열려 그대로 인쇄하거나 저장해 관리자에게 보고할 수 있습니다.
...
- **공용 가이드 컴포넌트**: 기존 업무 페이지 전용이던 가이드 모달을 `GuideModal`/`GuideButton`
  공용 컴포넌트로 일반화해, 제목과 내용만 넘기면 어느 화면에서도 재사용할 수 있게 만들었습니다.
- **전체 메뉴에 적용**: 학교 관리, 학생 정보 조회, 구글시트 가져오기, 개발자 대시보드, 내 계정
  설정, 학생/반/과목 관리, 내 담임반/담당과목, 리포트 프린트, 반별 작성 현황, 통계 대시보드,
...
- **@전체 멘션**: 채팅 입력창에서 `@`를 누르면 개인 이름들과 함께 "📢 전체"가 항상 맨 위에
  뜹니다. 선택하면 `@전체`가 입력되고, 메시지에서 주황색으로 강조되며 메시지 줄 배경도
  옅은 주황으로 표시돼 스크롤 중에도 눈에 띕니다. (참고: `#부서명`은 "다른 부서 채팅방에
  같은 글을 공유"하는 별개 기능인데, 지금 등록된 부서가 초등부 하나뿐이라 보낼 대상이 없어
  동작하지 않았던 것입니다 — 버그가 아니라 부서가 하나뿐이라 생긴 정상 동작입니다.)
- **채팅 지연 개선**: 메시지를 보내면 실시간 수신 이벤트를 기다리지 않고 전송 즉시 화면에
  바로 붙도록 바꿨습니다(낙관적 업데이트). 그동안은 실시간 연결이 늦게 오면 보낸 메시지가
  화면에 늦게 나타나는 것처럼 보였습니다. 추가로 브라우저 탭이 백그라운드에 있다가 다시
  보일 때마다(창 전환, 절전 복귀 등) 최신 메시지를 한 번 더 동기화하는 안전망도 넣었습니다.
- main에 배포한 뒤 채팅이 아예 안 올라온다는 신고가 있었는데, 코드 점검 결과 전송/수신
  로직과 실제 DB 컬럼이 정확히 일치해 코드상 원인은 찾지 못했습니다. Vercel Production
  환경변수(NEXT_PUBLIC_SUPABASE_URL 등)가 staging과 다르게 설정되어 있을 가능성을 안내
  드렸습니다 — 확인 부탁드립니다.
...
- **매뉴얼/실무자매뉴얼/학기관리 캐시 완화는 보류**: 원래 제안에는 있었지만, 확인해보니 이
  화면들은 로그인 세션(쿠키) 확인이 필요해 Next.js 구조상 `force-dynamic`을 빼도 실제로는
  캐시되지 않습니다(쿠키를 읽으면 항상 요청마다 새로 렌더링됨). 효과 없는 변경을 넣는 대신
  이번엔 적용하지 않았습니다.
- **대형 테이블 조회 컬럼 지정**: 주간 학생 관찰기록 보드, 업무 채팅(메시지/반응/읽음), 학생
  정보 조회 화면에서 전체 컬럼(`select *`) 대신 실제 화면에 쓰는 컬럼만 지정하도록 정리해
  전송량을 줄였습니다.

## v0.50.0 - 2026-08-04 (staging)

파일럿 테스트 전 마지막 정비입니다. 권한 체계를 다시 정리하고, 반 담임을 이름 기준으로 임시
배정할 수 있게 했고, 관리자 전용 메뉴 두 개(교육뉴스/GIA시스템)를 새로 추가했습니다.

**권한 재정비**
- 교사 계정은 이제 정말로 "자기 반/자기 담당과목"만 볼 수 있습니다 - 예전에는 URL을 직접
  입력하면 다른 반 학생의 리포트·부모 연락처를 보거나 PDF로 출력하거나 통합검색으로 찾아낼 수
  있었는데(`/weekly-report/students`, `/weekly-report/print`,
  `/weekly-report/students/[id]`, 검색, PDF 발급 라우트), 전부 소유권 검사를 추가해 막았습니다.
- 행정직원/관리자는 그대로 업무·실무자매뉴얼·운영관리·학교관리·주간학생관찰기록을 봅니다.
- 관리자(그리고 개발자)만 보는 새 "관리자" 메뉴를 만들어 관리자 대시보드/학교 현황판/교육뉴스/
  GIA시스템을 모았습니다. 문의및건의사항은 특정 직급 전용이 아니라서 관리자 메뉴에서 빼고
...
- AI가 web 검색으로 국제학교/교육정책/교육 트렌드 최신 소식을 찾아 정리하는 다이제스트입니다.
  매주 월·수요일 아침 자동 생성되고(Vercel Cron), 관리자가 원하면 언제든 바로 새로 만들 수도
  있습니다.

**신규: GIA시스템 (관리자 전용)**
- GIA가 이미 갖춘 운영 시스템(구비서류/내규/계약서/학생관리/교사관리/교직원관리/매뉴얼/
  운영계획안)을 카테고리별로 정리해 보여주고, AI가 다른 국제학교·공립학교 사례를 검색해 GIA에
  없는 시스템을 제안합니다. "운영관리 제안함으로 보내기"를 누르면 기존 제안함→채택예정→발행
  흐름을 그대로 타고, 발행되는 순간 이 표의 상태가 자동으로 "보유"로 바뀝니다.

**가벼운 정리**
- 채팅/업무/구글시트가져오기에서 저장 실패 시 Postgres 원본 에러 문구(영어)가 그대로 보이던
  것을 "일시적인 오류입니다. 잠시 후 다시 시도해주세요" 같은 한국어 안내로 통일했습니다.
- 안 쓰는 죽은 코드(`CompletionGauge.tsx`, `/weekly-report/admin/terms` 리다이렉트 페이지)를
  정리했습니다.
- `manifest.json`을 추가해 "홈 화면에 추가"가 실제로 앱처럼(standalone) 열리도록 했습니다.

## v0.49.1 - 2026-08-03 (staging)

실시간 채팅과 업무판을 정말 꼼꼼하게 다시 점검해서, 조용히 메시지/업무 내용이 사라지거나
잘못될 수 있는 지점들을 고쳤습니다.

**채팅**
- 메시지 전송이 실패해도(끊긴 와이파이 등) 알려주지 않고 입력창만 비워져서 메시지가 조용히
  사라지는 문제, 그리고 실패 시 전송 버튼이 계속 잠긴 채로 남는 문제를 고쳤습니다 - 이제
  실패하면 알림을 띄우고 입력 내용을 복원하며, 버튼도 다시 눌릴 수 있게 됩니다.
- 파일 첨부도 같은 이유로 캡션이 먼저 지워지던 것을 저장 성공 후로 미뤘고, 업로드는 됐는데
  메시지 저장이 실패한 경우 남는 "고아 파일"을 자동 정리하도록 했습니다.
- 이모지 반응을 아주 빠르게 두 번 누르면(추가 요청이 아직 끝나기 전에 다시 클릭) 취소가
  실제로는 아무 효과 없이 무시되던 버그를 고쳤습니다.
- 재연결(와이파이 순간 끊김 등) 시 최근 100건으로 화면 전체를 덮어써서, 검색으로 점프해
  보고 있던 예전 메시지가 화면에서 사라지던 문제를 고쳤습니다 - 이제 기존 화면 내용을 유지한
  채 최신 변경분만 얹습니다.
- 다른 사람이 메시지를 보낼 때마다 화면이 맨 아래로 튀어서, 위로 스크롤해 예전 대화를 읽던
  중에도 계속 방해받던 문제를 고쳤습니다 - 이제 이미 맨 아래 근처에 있을 때만 자동으로
  따라 내려갑니다.
- 채팅 검색이 메시지가 많이 쌓여도 느려지지 않도록 검색용 인덱스(pg_trgm)를 추가했습니다.

**업무판**
- 반복 업무를 업무 상세 팝업에서 완료 처리하면(칸반 드래그가 아니라) 반복이 조용히 끊기던
  문제를 고쳤습니다 - 두 곳 모두 같은 로직을 쓰도록 정리했고, 두 사람이 거의 동시에 완료
  처리해도 다음 회차가 중복 생성되지 않도록 DB에도 안전장치를 추가했습니다.
- 업무 상태 변경/업무 확인/담당자 태그/댓글 등록/업무 삭제가 실패했을 때 조용히 무시되던
  것을 고쳐서, 실패하면 알림을 띄우고 화면을 원래 상태로 되돌리도록 했습니다(끊긴 와이파이
  중에도 "저장된 것처럼 보이는데 실제로는 안 된" 상황이 생기지 않도록).
- 업무 상세 팝업에서 완료 처리해도 칸반 정렬 기준(position)이 함께 갱신되도록 맞췄습니다.

## v0.49.0 - 2026-08-03 (staging)

여러 사람이 동시에 데이터를 입력할 때 생길 수 있는 두 가지 실제 위험을 점검해서 고쳤습니다
(실시간 채팅/칸반/전체 Realtime 채널은 점검 결과 이미 안전하게 정리되어 있었고, 아래 두 곳만
"조용한 데이터 유실" 위험이 있었습니다).

- 업무 "확인"(acknowledged_by)/담당자 태그(assignee_emails): 지금까지는 각자의 화면이 계산한
  배열 전체를 그대로 저장해서, 전체공지 업무를 여러 명이 거의 동시에 "확인" 클릭하면 늦게
  도착한 저장이 먼저 도착한 사람의 확인 기록을 덮어써 조용히 사라질 수 있었습니다.
  `toggle_task_ack`/`toggle_task_assignee`라는 DB 함수를 새로 만들어 서버에서 한 번에 안전하게
  처리하도록 바꿨습니다 - 몇 명이 동시에 눌러도 모든 기록이 안전하게 남습니다.
- 주간 학생 관찰기록: 같은 학생·과목·주 리포트를 담임/과목교사가 동시에(또는 같은 사람이 두
  탭으로) 처음 열어 저장하면 중복 리포트가 생길 수 있었습니다. DB에 고유 제약을 추가하고
  저장 방식을 upsert로 바꿔 중복이 원천적으로 생기지 않게 했습니다(기존에 이미 생긴 중복이
  있다면 최신 것만 남기고 자동 정리됩니다).
- 그 외 실시간 채팅(ChatPanel)/칸반 드래그앤드롭/Realtime 채널 정리(구독 해제) 상태를 전수
  점검했습니다 - 13개 채널 모두 정상적으로 정리되고 있어 별도 수정은 필요 없었습니다. 채팅
  전송을 낙관적 렌더링으로 바꾸는 것과 tasks 관련 중복 구독 채널 통합은 체감 속도를 더
  개선할 수 있는 여지로 확인되어, 다음 개선 후보로 남겨두었습니다(자세한 내용은 답변 참고).

## v0.48.1 - 2026-08-03 (staging)

개발자 계정의 존재를 개발자 본인 외에는 아무도 알 수 없도록, 다른 사람이 볼 수 있는 화면에서
개발자 계정 관련 표시를 모두 없앴습니다(개발자 본인이 로그인했을 때 자기 사이드바에 뜨는
"개발자" 배지·메뉴는 원래부터 본인만 보이므로 그대로 유지됩니다).

- 사용자 관리(/admin/users): 개발자 계정을 승인 대기/승인됨/거절 목록 어디에도 표시하지 않도록
  서버에서부터 걸러서 내려보냅니다. 실시간 갱신(구독)에서도 개발자 계정 관련 이벤트는 무시하도록
  했습니다. 화면 안내 문구에 있던 개발자 이메일 주소 언급도 삭제했습니다.
- 사용자 관리: "개발자" 배지와, 그로 인해 다른 계정과 다르게 보이던 부분(직위 선택/차단 버튼이
  안 보이던 것)을 없애 모든 계정이 동일한 모습으로 보이게 했습니다.
- 학교 관리 대시보드(/school): 교직원 리스트/카운트에서도 개발자 계정을 완전히 제외했습니다.

## v0.48.0 - 2026-08-03 (staging)

개발자 대시보드(/dev)에서 AI 기능별로 실제 얼마나 과금되고 있는지 달러로 확인하고, 과금이
부담스러운 기능은 항목별로 켜고 끌 수 있게 만들었습니다. 개발자가 기능을 끄면 해당 기능은
Anthropic API를 아예 호출하지 않아 비용이 0원이 되고, 모든 직원의 사이드바 프로필 아래에
빨간 배너로 어떤 기능이 멈춰있는지 표시됩니다.

- "AI 사용량 & 예상 과금" 섹션에 Anthropic 공식 요금표(Sonnet 5 $2/$10, Haiku 4.5 $1/$5 · 백만
  토큰당) 기준 예상 과금액을 추가했습니다. 전체 합계와 기능(route)별 금액이 함께 보이고,
  기능 목록은 30일 호출횟수 대신 예상 과금액이 큰 순서로 정렬됩니다. Sonnet 5는 2026년 8월
  31일까지 도입 특가이고 이후 정가($3/$15)로 오른다는 안내 문구도 함께 넣었습니다.
- "AI 기능 on/off (과금 조절)" 섹션을 새로 만들어, 사건기록 정리 AI/회의록 다듬기 AI/매뉴얼
  초안작성 AI 등 총 16개 AI 기능을 기록함·매뉴얼·문서·제안함·채택예정·업무 4개 그룹으로 묶어
  각각 켜짐/일시정지중 스위치로 조절할 수 있게 했습니다.
- 새 `ai_feature_flags` 테이블을 만들고, 모든 Claude 호출이 거쳐가는 단일 지점인
  `callClaudeJson()`(src/lib/ai/claude.ts)에 게이트를 추가했습니다 - 기능이 꺼져 있으면
  Anthropic API를 호출하기 전에 바로 막아서 과금이 발생하지 않고, 사용자에게는 "현재 이 AI
  기능은 관리자에 의해 일시정지되어 있습니다"라는 안내 메시지가 표시됩니다.
- 사이드바 프로필 블록(로고 아래, 이메일 위) 바로 밑에 빨간 배너를 추가해, 개발자가 끈 AI
  기능이 있으면 모든 로그인 사용자에게 "⏸️ AI 기능 일시정지중"과 함께 어떤 기능인지 실시간으로
  보여줍니다.
- 토글/배너는 giamicro.com 전체가 조회(SELECT)할 수 있고, 켜고 끄는 것(수정)은 개발자 계정만
  가능하도록 RLS를 설정했습니다.

## v0.47.0 - 2026-08-03 (staging)

구글시트로 학교정보를 일괄 등록하는 화면(/school/import)을 새로 만들었습니다. 학교 관리 > 구글
시트로 가져오기 메뉴(관리자 전용) 및 학교 관리 대시보드 상단 버튼에서 들어갈 수 있습니다.

- 구글시트에서 표를 CSV로 다운로드하거나(파일 > 다운로드 > 쉼표로 구분된 값), 셀 범위를 그대로
  복사해서 붙여넣으면 자동으로 표를 읽어 미리보기를 보여줍니다. 붙여넣기는 탭 구분, CSV
  업로드는 쉼표 구분을 자동으로 인식합니다.
- 3개 탭으로 나뉘어 있습니다.
  - 🧑‍🏫 교직원 명단: 이메일/이름/소속/직위 - 미리 등록해두면 선생님이 구글 계정으로 처음
    로그인할 때 승인 대기 없이 바로 시스템을 쓸 수 있습니다.
  - 🏫 반 구성: 학년/반/담임 이메일/부담임 이메일 - 기존 반이면 담임·부담임을 갱신하고, 없는
    조합이면 새 반을 만듭니다.
  - 🎓 학생 명부: 이름/영어이름/학년/반/보호자 연락처 - 학년+반이 일치하면 자동으로 그 반에
    연결됩니다.
- 각 행마다 형식 오류(이메일 형식, 필수값 누락 등)를 미리보기에서 빨간색으로 표시해 잘못된
  행은 제외하고 정상 행만 가져올 수 있습니다.

## v0.46.0 - 2026-08-03 (staging)

전체 코드 최적화 점검 결과를 반영했습니다.

- 로고 이미지(사이드바/모바일 헤더/로그인/온보딩/승인대기 화면, 총 5곳)를 일반 `<img>`에서
  Next.js `<Image>`로 전환했습니다. 실제 표시 크기에 맞춰 자동으로 리사이즈·최신 이미지
  포맷(WebP/AVIF)으로 변환되어 전송 용량이 줄고, 로그인/앱 진입 화면 첫 로딩(LCP)이
  빨라집니다.
- 주간 학생 관찰기록의 반별 위젯(StudentReportBoard)에서 "학생마다 전체 리포트 목록을 다시
  훑어 이번 주 작성 상태를 찾는" 방식이었던 부분을, 리포트가 바뀔 때 한 번만 학생별로 미리
  정리해두는 방식으로 바꿨습니다. 학생 수 x 리포트 누적량에 비례해 느려지던 구조를 없애서,
  리포트 데이터가 쌓일수록 체감되는 속도 저하를 예방합니다.
- 기존에 이미 적용되어 있던 최적화(미들웨어 승인상태 캐싱, 로그인 유저 정보 요청 단위 캐싱,
  Supabase 클라이언트 싱글턴, PDF/리치에디터 등 무거운 라이브러리의 서버 전용 격리)는 점검
  결과 여전히 잘 동작 중임을 확인했습니다.

### 추가로 제안하는 속도 개선 방법 (코드 외 조치 필요)
- Vercel 프로젝트에 이미 연동된 Speed Insights 대시보드를 주기적으로 확인하면, 실제 사용자
  기준으로 어느 화면이 느린지 데이터로 파악할 수 있습니다.
- Supabase에서 자주 필터링되는 컬럼(app_users.email, tasks.department, wr_reports.term_id/
  student_id, chat messages.department 등)에 인덱스가 걸려 있는지 한 번 점검해보시는 걸
  추천드립니다 - 데이터가 늘어날수록 체감 효과가 커집니다.
- 채팅창(ChatPanel)은 메시지 목록이 많아질수록 입력할 때마다 전체 목록을 다시 그리는 구조라,
  다음 최적화 후보로 남겨뒀습니다(구조가 복잡해 이번엔 안전하게 손대지 않았습니다).

## v0.45.2 - 2026-08-03 (staging)

채팅 입력창 안내문구를 짧게 줄였습니다.

- "메시지 보내기 (Enter 전송·Shift+Enter 줄바꿈, **굵게** *기울임* ~~취소선~~ `코드`, @개인호출,
  #부서공지)" → "메시지 보내기 (Enter 전송·Shift+Enter 줄바꿈, @개인호출, #부서공지)"로 정리.
  서식 안내는 입력창 위 서식 아이콘으로 이미 보여주고 있어 placeholder에서는 뺐습니다.

## v0.45.1 - 2026-08-03 (staging)

주간 학생 관찰기록 반별 대시보드(학생 카드) 이름 표시를 개선했습니다.

- 학생 이름 아래 영어이름이 한 줄로 잘리지 않고(예전엔 말줄임표로 잘림) 줄바꿈되며, 글자
...
- 대시보드에서 현재 학기, 개설된 반 수, 과목 수, 교사 수, 교직원 수, 재학생 수를 카드로
  한눈에 보여주고, 그 아래에 반별 담임/부담임, 과목별 담당 교사·수강 인원, 교사 리스트,
  교직원 리스트, 재학생 요약(24명까지 미리보기 + 더보기 안내)을 함께 보여줍니다.
- 각 카드에는 반 관리/과목반 세팅/사용자 관리/학생 정보 조회 등 해당 관리 화면으로 바로
  이동하는 링크를 달아뒀습니다.
- 조회 권한은 기존과 동일하게 행정직원 이상(교사 계정은 접근 불가)입니다.
- (수정) 학기 카드는 연도를 작게 위에, 학기명(예: 여름캠프)은 줄바꿈해서 가운데 크게 보이도록
  바꿨습니다. 반/과목/교사/교직원/학생 목록도 글자가 옆으로 밀리지 않도록 줄바꿈+작은 글자
  크기를 공통으로 적용했고, 학생 목록에는 영어이름도 함께 표시됩니다.

## v0.44.0 - 2026-08-03 (staging)

UI/UX 정리 6건을 반영했습니다.

- 업무상황판/주간관찰기록 등 화면 크기를 바꿔도 라벨·뱃지·부서탭 글자가 어중간하게 줄바꿈되며
  밀리지 않도록 nowrap/줄임(ellipsis) 처리를 정리했습니다.
- 실시간 로그(🔔) 항목을 관리자이거나 그 로그를 남긴 본인이면 지울 수 있게 됐습니다(업무 상세
  패널의 코멘트/로그 항목도 동일).
- 실시간 로그 컴팩트 뷰가 한 줄에 하나씩, 딱 5줄만 보이는 고정 높이 영역으로 바뀌었고, 위로
  스크롤하면 그 이전 로그를 추가로 불러옵니다(캐시에 무한정 쌓이지 않도록 상한을 둠).
- 업무탭을 모바일(작은 화면)에서 열면, 마우스 드래그로 나누던 2단 레이아웃 대신 채팅/칸반/내
  업무 탭으로 전체 화면을 하나씩 보여줍니다. 채팅 헤더도 좁은 화면에서 안 밀리도록 정리했습니다.
