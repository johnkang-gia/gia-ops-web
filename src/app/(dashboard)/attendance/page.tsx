...
import { syncEntriesIntoRegister } from "@/lib/attendanceRegister";
import type { AttendanceRecord, WrClass, WrStudent } from "@/lib/types";
import type { PendingEntry } from "@/components/attendance/TodayNotices";
...
export default async function AttendancePage({
  ...
  const [{ data: classesData }, { data: usersData }, dayRes, pendRes] = await Promise.all([
    classesQuery.order("grade", { ascending: true }),
    supabase.from("app_users").select("email, name").eq("status", "approved"),
    supabase.from("school_days").select("day, is_school_day, closed_reason, label").eq("day", date).maybeSingle(),
    // 결석으로 읽었지만 **누구인지 못 가린** 연락. 특이사항 칸의 "결석인 것 같은데" 쪽입니다.
    // 자동으로 처리된 것과 처리 못한 것을 함께 보여줘야 오늘 할 일이 한눈에 들어옵니다.
    supabase
      .from("attendance_entries")
      .select("id, student_name, reason, raw_text, status, state, date_from, date_to")
      .eq("state", "확인필요")
      .eq("status", "결석")
      .lte("date_from", date)
      .gte("date_to", date),
  ...
  // 달력 표가 아직 없어도(마이그레이션 전) 화면은 열려야 합니다.
  if (dayRes.error) console.error("[출석부] 수업일 달력을 읽지 못했습니다:", dayRes.error.message);
  if (pendRes.error) console.error("[출석부] 확인 필요한 연락을 읽지 못했습니다:", pendRes.error.message);
  const todayRow = dayRes.data as { is_school_day: boolean; closed_reason: string | null; label: string | null } | null;
  ...
  const pendingEntries: PendingEntry[] = ((pendRes.data as { id: string; student_name: string; reason: string | null; raw_text: string | null }[] | null) ?? []).map(
    (e) => ({ id: e.id, studentName: e.student_name, reason: e.reason, rawText: e.raw_text }),
  );
  ...
  const todayRecRes = await supabase.from("attendance_records").select("*").eq("date", date);
  if (todayRecRes.error) console.error("[출석부] 오늘 출결을 읽지 못했습니다:", todayRecRes.error.message);
